...
class EuroCityPerson(BaseMetric):
    ...
    Args:
        ...
        num_ji_process (int): The number of processes to evaluation JI.
            Defaults to 10.
    """
    default_prefix: Optional[str] = 'eurocityperson'
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        for idx, result in enumerate (results):
            ...
            out_file = osp.join(jsonfile_prefix, basename.replace('.png', '.json'))
            frame = {'identity': 'frame'}
            frame['children'] = mock_detections
            json.dump(frame, open(out_file, 'w'), indent=1)
        return jsonfile_prefix

    def compute_metrics(self, results: list, day: str ='day',mode: str ='val') -> Dict[str, float]:
        ...
        outfile_prefix='./results/mock_detections/{}/{}/'.format(day, mode)
        gt_path = '/data2/lbx/DATA/ECP/{}/labels/{}'.format(day, mode)
        # convert predictions to coco format and dump to json file
        jsonfile_prefix = self.format_results(preds, outfile_prefix)
        res= eval(time=day, mode=mode, eval_type='pedestrian',det_path=jsonfile_prefix,gt_path=gt_path)
        # eval_results = {
        #     'Reasonable': res['reasonable'],
        #     'Reasonable_small': res['small'],
        #     'Reasonable_occ=heavy': res['occluded'],
        #     'all': res['all']
        # }
        return res

...
# from ..ECPB import statistics
from tools.ECPB import statistics
# from .statistics import MrFppi
# from mmdetection.mmdet.datasets.ECPB import statistics
# import statistics
from tools.ECPB.dataloader import load_data_ecp
from tools.ECPB.match import Evaluator, Result, compare_all
from tools.ECPB.params import ParamsFactory, IoU
...
def evaluate_detection(results_path, det_path, gt_path, det_method_name, eval_type='pedestrian'):
    ...
    return dict(reasonable=results[0],small=results[1],occluded=results[2],all=results[3])
...
def eval(time='day', mode='val', eval_type='pedestrian', det_path=None,gt_path=None):
    ...
    # gt_path = './datasets/pedestrian_datasets/EuroCity/ECP/{}/labels/{}'.format(time, mode)

    if det_path is None:
        det_path = '/home/lbx/lbx2/DDQ-ddq_detr/results/mock_detections/{}/{}/'.format(time, mode)

    if gt_path is None:
        gt_path = '/data2/lbx/DATA/ECP/{}/labels/{}'.format(time, mode)

    det_method_name = 'DDQ FCN'
    ...
    result=evaluate_detection(results_path, det_path, gt_path, det_method_name, eval_type)
    print(result)
    return result
    # print ('')
    # print ('# -----------------------------------------------------------------')
